...
def main(option="training",innotation=INNOTATION_2,postfix=POSTFIX_1_9_2,network_map=NETWORK_MAP_COMPACT_2,nodes_map=NODES_MAP_COMPACT_2):
    ...
    input_test=np.load(AUDIO_DIR+'test_data'+postfix+'.npy')
    groundtruth_test=np.load(CHORD_DIR+'test_groundtruth'+postfix+'.npy')
    if option=="training":
        ...
        train(neural_network,input_train,groundtruth_train,input_test,groundtruth_test,LEARNING_RATE_MAP,samples)
        
        neural_network.store(postfix)
    elif option=="validating":
        samples=input_test.shape[0]
        # divided_batch_num=math.ceil(samples/BATCH) #for GPU
        divided_batch_num=int(samples/BATCH) #for CPU
        neural_network=CNN_Audio(input_test.shape,chord_list,network_map,divided_batch_num,nodes_map)
        neural_network.load_params(postfix)
        res=predict(neural_network,input_test,groundtruth_test)
        print(res)
